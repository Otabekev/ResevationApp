...
async def get_public_business(business_id: int) -> dict:
    async with httpx.AsyncClient() as client:
        resp = await client.get(f"{BACKEND_URL}/businesses/{business_id}/public", timeout=10)
        resp.raise_for_status()
        return resp.json()
...
async def get_services(business_id: int) -> list[dict]:
    async with httpx.AsyncClient() as client:
        resp = await client.get(f"{BACKEND_URL}/businesses/{business_id}/services", timeout=10)
        resp.raise_for_status()
        return resp.json()
...
async def get_staff(business_id: int) -> list[dict]:
    async with httpx.AsyncClient() as client:
        resp = await client.get(f"{BACKEND_URL}/public/businesses/{business_id}/staff", timeout=10)
        resp.raise_for_status()
        return resp.json()
...
async def get_available_slots(
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.get(f"{BACKEND_URL}/availability", params=params, timeout=10)
        resp.raise_for_status()
        return resp.json()
